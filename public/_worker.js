...
export default {
  async fetch(request, env) {
    ...
    const isAuthorized = (req) => {
      ...
      return authHeader === 'Bearer motioncar-admin-token-9988';
    };
    ...
    try {
      ...
      if (path === '/api/login' && method === 'POST') {
        ...
        if (username === 'MotionCar@stenku' && password === 'admin123') {
          return new Response(JSON.stringify({ success: true, token: 'motioncar-admin-token-9988' }), {
            status: 200,
            headers: { 'Content-Type': 'application/json', ...corsHeaders },
          });
        }
